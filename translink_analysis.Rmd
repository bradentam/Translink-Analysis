...
#480 Line
data_480_num <- UBC_data %>% 
  filter(line_no == "480") %>%
  select(-Average_Fullness_to_UBC, -Average_Fullness_from_UBC) %>% 
  gather( "Type", "Value", -line_no, -Time_Range)

ggplot(data = data_480_num, aes(x = as.numeric(Time_Range), y = Value, color = Type, linetype = Type, shape = Type)) + 
  geom_line() +
  geom_point() +
  ylim(0, 130) + 
  labs(title = "Average Number of Passengers for the 480 Bus", x = "Time Range", y = "Average Peak Passengers") +
  scale_color_manual("Direction", values = c("red", "blue"), label = c("From UBC", "To UBC")) +
  scale_linetype_manual("Direction", values = c(1, 5), label = c("From UBC", "To UBC")) +
  scale_shape_manual("Direction", values = c(16, 17), label = c("From UBC", "To UBC")) +
  scale_x_discrete(limits = c(1,2,3,4), labels = c("6:00-9:00", "9:00-15:00", "15:00-18:00", "18:00-21:00"))

data_480_full <- UBC_data %>% 
  filter(line_no == "480") %>%
  select(-Average_Peak_Passengers_to_UBC, -Average_Peak_Passengers_from_UBC) %>% 
  gather( "Type", "Value", -line_no, -Time_Range)

ggplot(data = data_480_full, aes(x = as.numeric(Time_Range), y = Value, color = Type, linetype = Type, shape = Type)) + 
  geom_line() +
  geom_point() +
  ylim(0, 130) + 
  labs(title = "Average Fullness for the 480 Bus", x = "Time Range", y = "Average Fullness") +
  scale_color_manual("Direction", values = c("red", "blue"), label = c("From UBC", "To UBC")) +
  scale_linetype_manual("Direction", values = c(1, 5), label = c("From UBC", "To UBC")) +
  scale_shape_manual("Direction", values = c(16, 17), label = c("From UBC", "To UBC")) +
  scale_x_discrete(limits = c(1,2,3,4), labels = c("6:00-9:00", "9:00-15:00", "15:00-18:00", "18:00-21:00"))
...
```{r}
#To UBC
data_full_to <- UBC_data %>% 
  filter(line_no %in% UBC_lines) %>%
  select(-Average_Fullness_from_UBC, -Average_Peak_Passengers_to_UBC, -Average_Peak_Passengers_from_UBC) %>% 
  gather( "Type", "Value", -line_no, -Time_Range)

ggplot(data = data_full_to, aes(x = line_no, y = Value, color = factor(Time_Range))) + 
  geom_point(alpha = 0.5) +
  ylim(0, 130) + 
  labs(title = "Average Fullness for All Buses to UBC", x = "Bus Number", y = "Average Fullness") +
  scale_color_manual("Time Range", values = factor(data_full_to$Time_Range), labels = c("6:00-9:00", "9:00-15:00", "15:00-18:00", "18:00-21:00"))

ggplot(data = data_full_to, aes(x = Time_Range, y = Value, label = line_no, color = factor(line_no))) + 
  geom_text(size = 3) +
  ylim(0, 130) + 
  labs(title = "Average Fullness for All Buses", x = "Bus Number", y = "Average Fullness") +
  scale_x_discrete(limits = c(1,2,3,4), labels = c("6:00-9:00", "9:00-15:00", "15:00-18:00", "18:00-21:00"))

```
```{r}
#From UBC
data_full_from <- UBC_data %>% 
  filter(line_no %in% UBC_lines) %>%
  select(-Average_Fullness_to_UBC, -Average_Peak_Passengers_to_UBC, -Average_Peak_Passengers_from_UBC) %>% 
  gather( "Type", "Value", -line_no, -Time_Range)

ggplot(data = data_full_from, aes(x = line_no, y = Value, color = factor(Time_Range))) + 
  geom_point(alpha = 0.5) +
  ylim(0, 130) + 
  labs(title = "Average Fullness for All Buses from UBC", x = "Bus Number", y = "Average Fullness") +
  scale_color_manual("Time Range", values = factor(data_full_to$Time_Range), labels = c("6:00-9:00", "9:00-15:00", "15:00-18:00", "18:00-21:00"))
